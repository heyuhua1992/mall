              ...
              <li v-for="(item, index) in addressListFilter"
                ...
                <div class="addr-opration addr-set-default">
                  <a href="javascript:;"
                    ...
                    v-if="!item.isDefault" @click="setDefault(item.addressId)">
                    <i>设置为默认地址</i>
                  </a>
                </div>
...
export default {
  data () {
    return {
      addressList: [],
      limit: _LIMIT,
      checkIndex: 0, // 当前选中第几个地址
      isMdShow: false, // 是否显示模态框
      addressId: -1 // 删除的时候需要
      // selectedAddrId: -1 // 下一步用
    }
  },
  mounted () {
    this.init()
  ...
  computed: {
    ...
    selectedAddrId () {
      if (this.addressList.length) {
        return this.addressList[this.checkIndex].addressId
      } else {
        return -1
      }
    }
